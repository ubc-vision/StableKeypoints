...
def show_cross_attention(attention_store: AttentionStore, res: int, from_where: List[str], select: int = 0):
    ...
    for i in range(len(tokens)):
        ...
        plt.imsave(f'outputs/image_{i}.png', image)
        images.append(image)
...
def text2image_ldm_stable(
    ...
    # print(model.scheduler.timesteps)
    # print("model.scheduler.timesteps[-start_time:]")
    # print(model.scheduler.timesteps[-start_time:])
    # exit()
...
# image_path = "./example_images/human3.6m.png"
image_path = "/scratch/iamerich/prompt-to-prompt/example_images/PXL_20210919_212104706.MP.jpg"
# image_path = "/scratch/iamerich/prompt-to-prompt/example_images/378-3780801_david-murray-news-person-full-body-png.jpeg"
prompt = "a man standing with a dirt road in the background"
...
plt.imsave('outputs/image_gt.png', image_gt)
plt.imsave('outputs/image_enc.png', image_enc)

prompt = "a person standing in front of a dirt road"
...
plt.imsave('outputs/image_inv[0].png', image_inv[0])
exit()


# prompts = ["a man standing with a white background",
#            "a man standing in front of a paved road"
#         ]

# cross_replace_steps = {'default_': .8,}
# self_replace_steps = .5
# blend_word = ((('dirt',), ("paved",))) # for local edit. If it is not local yet - use only the source object: blend_word = ((('cat',), ("cat",))).
# eq_params = {"words": ("paved",), "values": (2,)} # amplify attention to the word "tiger" by *2 
...
#     plt.imsave(f'outputs/carpet_{index:02d}.png', im)
    
    
    
prompts = ["a man standing with a white background",
           "a robot standing with a white background"
        ]

cross_replace_steps = {'default_': .8, }
self_replace_steps = .6
blend_word = ((('man',), ("robot",))) # for local edit. If it is not local yet - use only the source object: blend_word = ((('cat',), ("cat",))).
...
for index, im in enumerate(images):
    plt.imsave(f'outputs/obama_{index:02d}.png', im)
exit()
    
    
prompts = ["a man standing in front of a dirt road",
           "pencil drawing of a man standing in front of a dirt road"
        ]

cross_replace_steps = {'default_': .8, }
self_replace_steps = .6
blend_word = None
eq_params = {"words": ("pencil", "drawing",  ), "values": (5, 5,)}  # amplify attention to the word "watercolor" by 5
 
controller = make_controller(prompts, False, cross_replace_steps, self_replace_steps, blend_word, eq_params)
images, _ = run_and_display(prompts, controller, run_baseline=False, latent=x_t, uncond_embeddings=uncond_embeddings)

for index, im in enumerate(images):
    plt.imsave(f'outputs/watercolor_{index:02d}.png', im)